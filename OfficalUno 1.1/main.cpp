...
int main(int argc, char *argv[])
{
    ...
    graphics.renderTexture(playText, 340, 450);
    SDL_Texture* quitText = graphics.renderText("QUIT", font, whiteColor);
    graphics.renderTexture(quitText, 340, 510);
    graphics.presentScene();
    bool endGame = false;
    srand(time(0));
    SDL_Event event;
    ...
    while (page1) {
        ...
        if(x>340 && x<431 && y>510 && y<543) {
                    playText = graphics.renderText("QUIT", font, blackColor);
                    graphics.renderTexture(playText, 340, 510);
                    graphics.presentScene();
                 }
        else {
            playText = graphics.renderText("QUIT", font, whiteColor);
                    graphics.renderTexture(playText, 340, 510);
                    graphics.presentScene();
        }
        switch (event.type) {
