...
void aiDrawing(SDL_Texture* pileImage, double xx, double yy) // ham may boc bai
{
    Mouse mouse;
    mouse.x = 500;
    mouse.y = 260;
...
bool isKeptCard(SDL_Texture* pileImage)
{
                 ...
                 if(y>=480 && y<=619)
                 {
                    ...
                    if(cumlativeCards == 0) {
                        ...
                        if(checkCard(player1.size() - 1, player1)) {
                                bool answer = isKeptCard(pileImage);
                                if(!answer) {
                                    moveCardToPile(player1.size() - 1, pileImage);
    ...
    if(index == -1) {
        ...
        if(pile.number == player[player.size() - 1].number
           ...
           || player[player.size() - 1].color == "draw4") {
            ...
            if(answer==1) {
                SDL_Delay(200);
                aiMatching(pileImage, x, y, player[player.size() - 1].number, player[player.size() - 1].color);
