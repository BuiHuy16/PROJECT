...
void startGame()
{
    ...
    for(int i=0;i<player1.size();i++) {
        myHand = getCard(player1[i].number, player1[i].color);
    ...
    while(player1.size() !=0 && player2.size() !=0 && player3.size() !=0 && player4.size() !=0) { //gameloop
        ...
        switch (abs(playerTurn) % numberOfPlayers + 1) {
            case 1:
                yourMatch(pileImage, event);
                break;
            case 2:
                aiMatch(pileImage, player2, 166, 260, event);
                break;
            case 3:
                aiMatch(pileImage, player3, 616, 40, event);
                break;
            case 4:
                aiMatch(pileImage, player4, 966.8, 260, event);
                break;
        }
