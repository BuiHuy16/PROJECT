...
void yourMatch(SDL_Event &event)
{
    ...
    while (flag) {
        ...
        if(canMatch || cumlativeCards==0) renderTime();
        presentScene();
        Paused(event);
        for(int i=0;i<myCard.size();i++) {
        ...
        else if(event.type == SDL_MOUSEBUTTONUP || timeLeft==0) {
            if(event.button.button == SDL_BUTTON_LEFT || timeLeft==0) {
                 if(y>=480 && y<=619)
                 {
                     index = (x-300)/dd;
                 ...
                 else if((x>=500 && x<=590 && y>=260 && y<=399) || timeLeft==0) {
                    if(cumlativeCards == 0) {
                        drawCard(player1);
                        drawingCard(event);
                        arrangeCard(event);
