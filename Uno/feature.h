...
int findMinOfCards(int id)
{
    int res = 100;
    int idMin=1;
    if(player1.size()<res && id!=1) {
        res = player1.size();
        idMin = 1;
    }
    if(player2.size()<res && id!=2) {
        res = player2.size();
        idMin = 2;
    }
    if(player3.size()<res && player3.size()!=0 && id!=3) {
        res = player3.size();
        idMin = 3;
    }
    if(player4.size()<res && player4.size()!=0 && id!=4) {
        res = player4.size();
        idMin = 4;
    }
    return idMin;
...
bool gameOver()
{
    if(numberOfPlayers==2) {
        if(player1.size()==0||player2.size()==0) return true;
    }
    else if(numberOfPlayers==3) {
        if(player1.size()==0||player2.size()==0||player3.size()==0) return true;
    }
    else if(numberOfPlayers==4) {
        if(player1.size()==0||player2.size()==0||player3.size()==0||player4.size()==0) return true;
    }
    return false;
}
void resetFeature()
