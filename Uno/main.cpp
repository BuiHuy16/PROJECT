...
int main(int argc, char *argv[])
{
    ...
    if(matchClick != nullptr) Mix_FreeChunk(matchClick);
    if(drawClick != nullptr) Mix_FreeChunk(drawClick);
    if(skipWav != nullptr) Mix_FreeChunk(skipWav);
    if(reverseWav != nullptr) Mix_FreeChunk(reverseWav);
    if(plusingSound != nullptr) Mix_FreeChunk(plusingSound);
    if(bePlusedSound != nullptr) Mix_FreeChunk(bePlusedSound);
    if(winningSound != nullptr) Mix_FreeChunk(winningSound);
    if(losingSound != nullptr) Mix_FreeChunk(losingSound);
    TTF_CloseFont(font);
    TTF_CloseFont(font2);
    ...
    SDL_DestroyTexture(blackTriangle);
    SDL_DestroyTexture(blackTriangle1);
    SDL_DestroyTexture(blackTriangle2);
