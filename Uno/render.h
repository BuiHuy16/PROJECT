...
#include "feature.h"
void renderCard() // ham nay chi cout tren terminal
{
    for(auto x:player1) cout<<x.number<<" "<<x.color<<"  ";
    cout<<endl;
    for(auto x:player2) cout<<x.number<<" "<<x.color<<"  ";
    cout<<endl;
    if(numberOfPlayers>2) for(auto x:player3) cout<<x.number<<" "<<x.color<<"  ";
    cout<<endl;
    if(numberOfPlayers>3) for(auto x:player4) cout<<x.number<<" "<<x.color<<"  ";
    cout<<endl;
}
...
{
    if(pile.color == "wild") {
        if(tmpColor=="green") fillColor( 0, 255, 0, 255);
        else if(tmpColor=="blue") fillColor( 100, 100, 255, 255);
        else if(tmpColor=="red") fillColor( 255, 0, 0, 255);
        else if(tmpColor=="yellow") fillColor( 255, 255, 0, 255);
    }
    ...
    SDL_Texture* myName = renderText("You", fontNum, blackColor);
    renderTexture(myName, 190, 596);
   ...
   if(!aiWin[0]) getSecretCard(161, 272, 70, 103);
    ...
    SDL_Texture* p2Name = renderText(namePlayer[0], fontNum, blackColor);
    renderTexture(p2Name, 50, 375);
    ...
    if(numberOfPlayers>2) {
        ...
        SDL_Texture* p3Name = renderText(namePlayer[1], fontNum, blackColor);
        renderTexture(p3Name, 500, 150);
        SDL_DestroyTexture(numberCardP3);
    }
    if(numberOfPlayers>3) {
        ...
        SDL_Texture* p4Name = renderText(namePlayer[2], fontNum, blackColor);
        renderTexture(p4Name, 1050, 375);
        SDL_DestroyTexture(numberCardP4);
    }
    ...
    SDL_DestroyTexture(numberCardP1);
    SDL_DestroyTexture(numberCardP2);
}
...
void renderTime()
{
    SDL_Rect screen = {250, 30, timeLeft/5, 50};
